...
function customReshape(text) {
  ...
  const text = "نص تجريبي هنا";
  const reshapedText = customReshape(text);

  const levels = engine.getEmbeddingLevels(reshapedText, 'rtl');
  const visual = engine.getReorderedString(reshapedText, levels);

  const manualReverse = reshapedText.split('').reverse().join('');

  console.log("Visual:", visual);
  console.log("Manual Reverse:", manualReverse);
}
